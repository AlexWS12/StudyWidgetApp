...
class PatternAnalyzer:
    ...
    def generate_session_report(self, session_id: int, output_path: str = None) -> str | None:
        """Generate a per-session distraction summary report."""
        cursor = self.db.cursor()
        cursor.execute('''
        ...
        # Map DB column names to display labels for the counts row
        col_to_label = {
            "phone_distraction":     "Phone",
            "look_away_distraction": "Looking Away",
            "left_desk_distraction": "Left Desk",
            "app_distraction":       "App Switch",
        ...
        if output_path is None:
            output_path = f"session_{session_id}_report.md"
        ...
        _a(f"# Session Report — Session #{session_id}\n")
        _a(f"> Generated on {datetime.now().strftime('%B %d, %Y at %I:%M %p')}\n")
        _a("---\n")

        _a("## Session Overview\n")
        _a("| Metric | Value |")
        _a("|--------|-------|")
        _a(f"| Start Time | {start_str} |")
        _a(f"| End Time | {end_str} |")
        _a(f"| Total Duration | {duration} s ({round(duration / 60, 1)} min) |")
        _a(f"| Total Focused Time | {focused_time} s ({round(focused_time / 60, 1)} min) |")
        _a(f"| Total Distraction Time | {distraction_time} s ({round(distraction_time / 60, 1)} min) |")
        _a("")

        _a("## Distraction Count by Type\n")
        _a("| Type | # of Distractions |")
        _a("|------|------------------|")
        ...
        if event_durations:
            _a("## Duration of Each Distraction Event (seconds)\n")
            for key, label in col_to_label.items():
                durations = event_durations.get(key)
                if not durations:
                    continue
                _a(f"### {label}\n")
                _a("| Event # | Duration (s) |")
